...
def restore_array(shuffled_array, indices):
    """
    对给定的一维数组进行复原，根据提供的索引，返回原始顺序的数组。

    参数：
    shuffled_array: numpy数组，已经随机置乱的数组。
    indices: numpy数组，用于复原的索引。

    返回值：
    restored_array: numpy数组，根据提供的索引复原后的数组。
    """
...
def decrypt_by_model(info,filename,blocks):
    ...
    print("极化码加密之后的信息shape:"+str(info.shape))
...
def ours_experiment_chessboard(bit, dir_path, pixel_cnt, block_size, model_name,masking_rate):
    ...
    for filename in os.listdir(dir_path):
        ...
        avail_pix_num = int(512 * 512 * 0.25)
        N = 16384 # polar 码长
        crossover_p = crossover_ps[cnt]
        K = calculate_K(N,crossover_p) # 计算理论值
        print(K)
        cnt += 1
        [mess,encoded_mess]  = call_function_with_arr(matlab_dir_path, 'polar_encode_python', 2, avail_pix_num, N, K, crossover_p)
        print("原始信息"+str(mess))
        # mess是原始秘密信息，encoded_mess是编码后的秘密信息
        encoded_mess = np.array(encoded_mess).flatten()
        decrypt_result = decrypt_by_model(encoded_mess,filename,blocks)
        print(decrypt_result.shape)
        z = call_function_with_arr(matlab_dir_path, 'polar_decode_python', 1, decrypt_result, N, K, crossover_p)
        print("解密信息："+str(z))
