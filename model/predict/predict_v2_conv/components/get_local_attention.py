...
def extract_seq_patches(x, kernel_size, rate):
    ...
    x = K.temporal_padding(x, (p_left, p_right))  # 保证滑动不超过范围
    xs = [x[:, i: i + seq_len] for i in range(0, k_size, rate)]  # 有点像阶梯式的基于x生产了三个序列
...
class LocalSelfAttention(OurLayer):
    ...
    def __init__(self, heads, size_per_head, neighbors=1, rate=1, block_size=6, d_encoder=4,filter=16,
                 key_size=None, mask_right=False, **kwargs):
    ...
    def build(self, input_shape):
        ...
        self.attention = MultiHeadAttention(
            ...
            value_dim=4,
            num_heads=1,
            name="local_multi_head_1"
        )
        self.reshape = Reshape((int(self.block_size * self.block_size/2), self.filter * 4))
...
class LocalSelfAttention2(OurLayer):
    """局部多头自注意力机制
    说明：每个元素只跟相对距离不超过neighbors的元素有关联，这里的rate
    是真正的膨胀率（跟膨胀卷积一样），如果不了解可以忽略，默认为1就好。
    """

    def __init__(self, heads, size_per_head, neighbors=1, rate=1, block_size=6, d_decoder=16,filter = 16,
                 key_size=None, mask_right=False, **kwargs):
        super(LocalSelfAttention2, self).__init__(**kwargs)
        self.attention = None
        self.heads = heads
        self.size_per_head = size_per_head
        self.out_dim = heads * size_per_head
    ...
    def build(self, input_shape):
        super(LocalSelfAttention2, self).build(input_shape)
        if self.mask_right:
            mask_right = np.ones((1, 1 + 2 * self.neighbors))
            mask_right[:, - self.neighbors:] = 0
        else:
        ...
        self.attention = MultiHeadAttention(
            ...
            value_dim=8,
            num_heads=1,
            name="local_multi_head_22"
        )
        self.reshape = Reshape((int(self.block_size * self.block_size), self.filter*  3))
    ...
    def call(self, inputs):
        if isinstance(inputs, list):
            ...
            x, x_mask = inputs, None
        # 提取局部特征
        kernel_size = 1 + 2 * self.neighbors  # 想象成一维序列会稍微好理解一点
        xp = extract_seq_patches(x, kernel_size, self.rate)
        # 变换shape
        ...
        # print(x.shape)
        # print(xp.shape)
        x = self.attention(x, xp, xp)  # 做attention
        # 恢复shape
...
class SparseSelfAttention(OurLayer):
    ...
    def call(self, inputs):
        ...
        if self.mask_right:
            ...
            mask = (ones - matrix_band_part(ones, -1, 0)) * 1e10
            a = a - mask
        # Attention2
