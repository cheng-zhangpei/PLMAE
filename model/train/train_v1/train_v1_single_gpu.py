...
def train_model(epochs, batch_size, x, y, model, model_name, initial_learning_rate):
    ...
    decay_rate = 0.96
    early_stop = EarlyStopping(monitor='val_mae', min_delta=0.001, patience=5, mode='min',
    ...
    save_path = "./trained_model/" + model_name + ".h5"
...
def train_model_with_spatial_info(epochs, batch_size, x1, x2, y, model, model_name, initial_learning_rate):
    ...
    decay_rate = 0.95
    early_stop = EarlyStopping(monitor='val_mae', min_delta=0.0001, patience=60, mode='min',
                               restore_best_weights=True)
    ...
    predict_res = model.predict([x1_test, x2_test])
