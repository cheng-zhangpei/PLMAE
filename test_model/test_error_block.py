...
def ours_experiment_chessboard(bit, dir_path, pixel_cnt, block_size, model_name):
    ...
    for filename in os.listdir(dir_path):
        # for m in range(100):
            ...
            decoder_outputs, spatial_mask_infos, chessboard_blocks = masking_block(encrypt_blocks)
            ...
            # save_path_predicted = "../interface/predicted_image/" + str(filename_without_extension) + "/predict/" + str(
            #     filename_without_extension) + "-" + str(m) + ".bmp"
            # save_path_masked = "../interface/predicted_image/" + str(filename_without_extension) + "/masked/" + str(
            #     filename_without_extension) + "-" + str(m) + ".bmp"
            ...
            # 干脆在这个地方计算一下mae算了
            # model_dis,tra_dis = get_distance(tra_ori_pixels, predict_tra, predict_result)
            # print(np.average(model_dis))
            # print(np.average(tra_dis))
            # print("---------")
            # tra_cnt,model_cnt = analysis(tra_dis,model_dis)
            # print(model_cnt)
            # print(tra_cnt)
            # print("================================")
