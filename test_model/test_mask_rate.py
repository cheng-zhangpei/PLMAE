"""
@Function: 修改掩码方案以及掩码率
@Author : ZhangPeiCheng
@Time : 2024/2/13 12:03
"""
import pandas as pd
...
from train_set_build.train_change_rate import masking_block
...
def generate_binary_stream_change(block_size,block_cnt):
    # cnt是要生成的0和1的个数
    # 返回一个由0和1组成的字符串
    stream = []
    seq_cnt = int((block_size * block_size)*0.75)  # 每一块需要承载的信息数量
    # print(seq_cnt)
...
def get_model_test_error(model_name):
    ...
    encoder_input, decoder_output,spatial_mask_infos_all = get_training_set(directory_name, 15, 1000, 6)
    model = load_model(model_name)
    test_res = model.evaluate([encoder_input, spatial_mask_infos_all],decoder_output)
    return test_res[1]

def encrypt_chessboard_change(image_blocks, bit, info_for_block):
    """
    第一个是预留像素，第二个是加密像素，这样的顺序进行
    嵌入规则：如果info[j] == 1 则翻转bit位，0则不进行翻转
    ...
    for i in range(len(image_blocks)):
            ...
            for j in range(n):
                ...
                if (is_odd_row and is_odd_column) and is_odd_row:
                    ...
                    if info[cnt] == 1:
                        # 将嵌入信息的像素放到图像块中去
                        chessboard_block[i, j] = image_block[i, j] ^ bit
                        encrypt_pixel.append(chessboard_block[i, j])
                    else:
                        chessboard_block[i, j] = image_block[i, j]
                        encrypt_pixel.append(chessboard_block[i, j]) 
                    cnt += 1
        ...
        origins.append(origin)
    return np.array(encrypt_pixels), np.array(chessboard_blocks), np.array(origins)
def recover_image(predict_value, chessboards,save_path_predict):
    predict_blocks = []
    masked_blocks = []
    ...
    for k in range(len(chessboards)):
        ...
        for i in range(len(chessboard)):
            for j in range(len(chessboard[0])):
                if chessboard[i,j] == -1:
                    predicted_block[i,j] = predict_pixels[cnt]
    ...
    df_predict = pd.DataFrame(predicted_image)
    df_predict.to_excel(save_path_predict,index=False)
...
def ours_experiment_chessboard(bit, dir_path, pixel_cnt, block_size, model_name):
    ...
    for filename in os.listdir(dir_path):
            ...
            info = generate_binary_stream_change(block_size, int(len(blocks)))
            encrypt_pixels, encrypt_blocks, origin = encrypt_chessboard_change(blocks, bit, info)
            # print("encrypt_pixels")
            # print(encrypt_pixels[100])# 加密后的像素
            # print(encrypt_blocks[100]) # 加密之后的像素块
            # print(origin[100])# 原始像素值
            ...
            decoder_outputs, spatial_mask_infos, chessboard_blocks, origin_block = masking_block(encrypt_blocks)
            # print("input model")
            # print(chessboard_blocks[1000])
            # print(spatial_mask_infos[1000])
            ...
            # print(predict_result[100].flatten())
            # print("decrypt result")
            # print(decrypt_result[100])
            # print("origin info")
            # print(info[100])
            # print("shape test")
            # print(decrypt_result[100].shape)
            # print(len(info[100]))
            # print(predict_result[100].shape)
            # print(origin[100].shape)
            # print(encrypt_pixels[100].shape)
            filename_without_extension = filename.rsplit(".", 1)[0]

            save_path_predicted = "../interface/predicted_image/"+str(filename_without_extension)+"-"+str(1)+".xlsx"

            # decrypt_result_tra = calc_res(bit, en_selected_pixel_, predict_tra)
            # save_path_predicted = "./predicted_image/predict/" + str(filename_without_extension)+"-"+str(m)+".xlsx"
            # save_path_masked = "./predicted_image/masked/" + str(filename_without_extension)+"-"+str(m)+".xlsx"
            recover_image(predict_result, chessboard_blocks, save_path_predicted)
