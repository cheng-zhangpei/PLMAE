...
from train_set_build.train_conv_set import open_picture, masking_block, random_block_split, get_training_set
...
def optic_model(predict_result,chessboard):
    predict_result = np.array(predict_result)
    m,n,l = predict_result.shape
    predict_result = predict_result.reshape((m,n * l))
    predict_result_optimistic = []
    for i in range(len(chessboard)):
        block = chessboard[i]
        block = np.pad(block,pad_width=1,constant_values=0)
        fill_block = fill_image_block(block)
        predict_for_block = predict_result[i]
        cnt = 0
        for j in range(1,len(fill_block)-1):
            for k in range(1,len(fill_block[0])-1):
                if fill_block[j, k] == -1:
                    complex = get_complex(fill_block, (j, k))
                    if complex <= 30:
                        predict_for_block[cnt] = Chen_predict(fill_block,(j,k))
                    cnt += 1
        predict_result_optimistic.append(predict_for_block)
    return np.array(predict_result_optimistic)
...
def ours_experiment_chessboard(bit, dir_path, pixel_cnt, block_size, model_name):
    ...
    for filename in os.listdir(dir_path):
        ...
        select_block_(blocks, block_indexes, filename)
        ...
        predict_result_optimistic = optic_model(predict_result,chessboard_blocks)
        decrypt_result = get_residual_result_(predict_result, encrypt_pixels, bit)
        decrypt_result_optic = get_residual_result_(predict_result_optimistic, encrypt_pixels, bit)
        # 算complex并进行定点的覆盖
        ...
        error_rate_op, error_cnt_op = error_judge(decrypt_result_optic.flatten(), info)
        ...
        print("图像:" + str(filename) + "优化后模型错误率:" + str(error_rate_op) + "," + "错误像素数量: " + str(error_cnt_op))
